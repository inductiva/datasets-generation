...
flags.DEFINE_string('OBJ_DIR', 'data/output/instant-mesh-large/meshes_processed', 'Object directory')
flags.DEFINE_string('SUBMISSIONS_FILE', 'data/submissions/submissions.jsonl', 'Submissions file')
...
def main(_):
    ...
    for i in tqdm(range(FLAGS.NUM_OBJECTS), desc="Processing Objects"):
        ...
        rotation_angles = [int(random.uniform(0, 360)) for _ in range(FLAGS.NUM_ROTATION_VALUES - 2)]
        rotation_angles.extend([0, 180])

        wind_speeds = [int(random.uniform(FLAGS.MIN_WIND_SPEED, FLAGS.MAX_WIND_SPEED)) for _ in range(FLAGS.NUM_WIND_SPEED_VALUES)]

        for j in tqdm(range(FLAGS.NUM_WIND_SPEED_VALUES), desc="Processing different wind speeds"):
            for k in tqdm(range(FLAGS.NUM_ROTATION_VALUES), desc="Processing different rotation values"):
                data = {
                    'wind_speed': wind_speeds[j % FLAGS.NUM_WIND_SPEED_VALUES],
                    'rotate_angle': rotation_angles[k % FLAGS.NUM_ROTATION_VALUES],
                ...
                with open(FLAGS.SUBMISSIONS_FILE, 'a') as file:
