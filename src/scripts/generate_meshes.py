...
MESHES_PROCESSED_DIR = os.path.join(OUTPUT_PATH, OUTPUT_SUBDIR, 'meshes_processed')
COMMAND_TEMPLATE = 'python run.py {config} {input_file} --save_video --output_path {output_path}'
...
def generate_meshes(input_path, config_file):
    ...
    image_files = sorted([f for f in files if f.endswith('.jpg') or f.endswith('.png')], key=natural_sort_key)
    ...
    for i, filename in tqdm(enumerate(image_files)):
        ...
        command = COMMAND_TEMPLATE.format(config=config_file, input_file=image_path, output_path=OUTPUT_PATH)
